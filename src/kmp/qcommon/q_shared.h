...
//::KUA.cmt:   Q_EXPORT = Function that always exports as visible (even with -fvisibility=hidden)
#if (defined _MSC_VER)
...
vec_t VectorLengthSquared2D(vec2_t const v);  //::KUA.add
...
//
// sk.chg-> Were numbers, shifted to `<<` for clarity
...
#define BUTTON_WALKING      (1<<4)  // walking can't just be inferred from MOVE_RUN
                                    // A key pressed late in the frame will only generate a small move value for that frame
